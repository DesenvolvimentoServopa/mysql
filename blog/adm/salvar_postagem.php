...
if($_GET['pagina'] == 4){//apenas uma imagem
    ...
    if($_POST['tipo_postagem'] == 1){
        ...
        $result_tipo = $conn->query($tipo_postagem);
        ...
        while($row_tipo = $result_tipo->fetch_assoc()){
            ...
            $result_alter = $conn->query($alter_tipo) or die(mysqli_error($conn));
        }
    }

    //salvando no banco de dados a nova postagem

    $postagem_insert = "INSERT INTO blog_post (id_post_user, 
                                                tipo_postagem, 
                                                tipo_arquivo, 
                                                titulo, 
                                                mensagem, 
                                                file_img, 
                                                data,"; 
    $postagem_insert .= empty($_POST['dataFim']) ? "" : 'data_drop,';        
                            $postagem_insert .= "alerta_comentario)
                        VALUE ('".$_SESSION['id']."',
                                '".$_POST['tipo_postagem']."',
                                '".$tipo_file."',
                                '".$_POST['titulo']."',
                                ...
                                '".$caminho_db."',
                                '".$data."',";
    $postagem_insert .= empty($_POST['dataFim']) ? "" : "'".$_POST['dataFim']."',";
    $postagem_insert .=         "'".$_POST['alertar_comentario']."')";

    $resul_insert = $conn->query($postagem_insert)or die(mysqli_error($conn));
    ...
    header('location: dashboard.php?pagina=1&msn=5');
        ...
        //salvando no banco de dados a nova postagem
        $postagem_insert = "INSERT INTO blog_post (
                                    id_post_user, 
                                    tipo_postagem, 
                                    titulo, 
                                    mensagem, 
        ...
        while ($_FILES['file'.$contIMG.'']['type'] != NULL){
            ...
            $insertPostagem = $conn->query($filePostagem) or die(mysqli_error($conn));
